...
<table class="datatable3"  border="1" style="width:160%">
    ...
    <tr style=" background-color:white; color:black; font-size:12;" >
      ...
      <th><?php if(!empty($sa)){echo uang($sa);} ?></th>
      <th  style="border:none; background-color:white;"></th>
      <th  style=" text-align:right; background-color:orange; color:white; font-size:12;"><?php if(!empty($kertas)){echo uang($kertas);} ?></th>
      <th  style=" text-align:right; background-color:orange; color:white; font-size:12;"><?php if(!empty($logam)){echo uang($logam);} ?></th>
      <th  style=" text-align:right; background-color:orange; color:white; font-size:12;"><?php if(!empty($giro)){echo uang($giro);} ?></th>
			<th  style=" text-align:right; background-color:orange; color:white; font-size:12;"><?php if(!empty($transfer)){echo uang($transfer);} ?></th>
  ...
  <?php
    ...
    while (strtotime($dari) <= strtotime($sampai)) {
      $qpengeluaran = "SELECT SUM(uang_logam) as setoranlogam,SUM(uang_kertas) as setorankertas,SUM(giro) as setorangiro,SUM(transfer) as setorantransfer FROM setoran_pusat WHERE tgl_setoranpusat ='$dari' AND kode_cabang='$cb[kode_cabang]' AND status='1' GROUP BY tgl_setoranpusat";
      $pengeluaran  = $this->db->query($qpengeluaran)->row_array();
        
      $qpenerimaan  = "SELECT SUM(setoran_logam) as lhplogam, SUM(setoran_kertas) as lhpkertas, SUM(setoran_bg) as lhpgiro,SUM(setoran_transfer) as lhptransfer,SUM(girotocash) as lhpgirotocash FROM setoran_penjualan WHERE tgl_lhp ='$dari' AND kode_cabang='$cb[kode_cabang]' GROUP BY tgl_lhp";
      ...
      $qgl          = "SELECT SUM(jumlah_logamtokertas) as jmlgantikertas FROM logamtokertas WHERE tgl_logamtokertas='$dari' AND kode_cabang='$cb[kode_cabang]' GROUP BY tgl_logamtokertas";
    ...
    ?>
	    <tr>
	      ...
	      <td align="right" style="font-weight:bold; color:green"><?php if(!empty($lhpkertas)){echo uang($lhpkertas);} ?></td>
	      <td align="right" style="font-weight:bold; color:green"><?php if(!empty($lhplogam)){echo uang($lhplogam);} ?></td>
	      <td align="right" style="font-weight:bold; color:green"><?php if(!empty($penerimaan['lhpgiro'])){echo uang($penerimaan['lhpgiro']);} ?></td>
				<td align="right" style="font-weight:bold; color:green"><?php if(!empty($penerimaan['lhptransfer'])){echo uang($penerimaan['lhptransfer']);} ?></td>
	      <td align="right" style="font-weight:bold; color:green"><?php if(!empty($totallhp)){echo uang($totallhp);} ?></td>
	      <td align="right" style="font-weight:bold; color:red"><?php if(!empty($pengeluaran['setorankertas'])){echo uang($pengeluaran['setorankertas']);} ?></td>
	      <td align="right" style="font-weight:bold; color:red"><?php if(!empty($pengeluaran['setoranlogam'])){echo uang($pengeluaran['setoranlogam']);} ?></td>
        <td align="right" style="font-weight:bold; color:red"><?php if(!empty($pengeluaran['setorangiro'])){echo uang($pengeluaran['setorangiro']);} ?></td>
	      <td align="right" style="font-weight:bold; color:red"><?php if(!empty($pengeluaran['setorantransfer'])){echo uang($pengeluaran['setorantransfer']);} ?></td>
			
	      <td align="right" style="font-weight:bold; color:red"><?php if(!empty($totalsetoranbank)){echo uang($totalsetoranbank);} ?></td>
	      <td align="right" style="font-weight:bold; color:black; color:blue"><?php if(!empty($saldo)){echo uang($saldo);} ?></td>
	      <td style="border:none; background-color:white;"></td>
	      <td align="right" style="font-weight:bold;"><?php if(!empty($totalrinciankertas)){echo uang($totalrinciankertas);} ?></td>
	      <td align="right" style="font-weight:bold;"><?php if(!empty($totalrincianlogam)){echo uang($totalrincianlogam);} ?></td>
		
	      <td align="right" style="font-weight:bold;"><?php if(!empty($totalrinciangiro)){echo uang($totalrinciangiro);} ?></td>
        <td align="right" style="font-weight:bold;"><?php if(!empty($totalrinciantrf)){echo uang($totalrinciantrf);} ?></td>
	      <td align="right" style="font-weight:bold; color:blue"><?php if(!empty($totalrincianfisik)){echo uang($totalrincianfisik);} ?></td>
	      <td align="right" style="font-weight:bold;"><?php if(!empty($gl['jmlgantikertas'])){echo uang($gl['jmlgantikertas']);} ?></td>
	      <td align="right" style="font-weight:bold;"><?php if(!empty($penerimaan['lhpgirotocash'])){echo uang($penerimaan['lhpgirotocash']);} ?></td>
