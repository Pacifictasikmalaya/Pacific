...
$dpbkembaliab = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
WHERE tgl_pengembalian >= '$ab[tanggal]' AND tgl_pengembalian <= '$hariini'
AND `kode_produk` = 'AB' AND kode_cabang='$r->kode_cabang'")->row_array();
...
$dpbkembaliar = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
$dpbkembalias = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
$dpbkembalibb = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
$dpbkembalicg = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
$dpbkembalicgg = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
$dpbkembalidep = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
$dpbkembalidk = $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
$dpbkembalids= $this->db->query("SELECT
TRUNCATE(SUM(jml_pengembalian),2) as jumlah 
FROM
`detail_dpb`
JOIN `dpb` ON `detail_dpb`.`no_dpb` = `dpb`.`no_dpb`
...
AND `kode_produk` = 'DS' AND kode_cabang='$r->kode_cabang'")->row_array();
  ...
  $sab = ($ab['jumlah']/30) ; 
  $sar = ($ar['jumlah']/240) + ($msjar['jumlah']/240) -$dpbambilar['jumlah'] + $dpbkembaliar['jumlah'] ;
  $sas = ($as['jumlah']/36) + ($msjas['jumlah']/36) -$dpbambilas['jumlah'] + $dpbkembalias['jumlah'] ;
  $sbb = ($bb['jumlah']/20) + ($msjbb['jumlah']/20) -$dpbambilbb['jumlah'] + $dpbkembalibb['jumlah'] ;
  $scg  = ($cg['jumlah']/10) + ($msjcg['jumlah']/10) -$dpbambilcg['jumlah'] + $dpbkembalicg['jumlah'] ;
  $scgg = ($cgg['jumlah']/10) + ($msjcgg['jumlah']/10) -$dpbambilcgg['jumlah'] + $dpbkembalicgg['jumlah'] ; 
  
  $sdep = ($dep['jumlah']/10) + ($msjdep['jumlah']/10) -$dpbambildep['jumlah'] + $dpbkembalicgg['jumlah'] ; 
  $sdk  = ($dk['jumlah']/30) + ($msjdk['jumlah']/30) -$dpbambildk['jumlah'] + $dpbkembalidk['jumlah'] ; 
  $sds  = ($ds['jumlah']/30) + ($msjds['jumlah']/30) -$dpbambilds['jumlah'] + $dpbkembalids['jumlah'] ;  
 ...
 ?>
  <tr>
    ...
    <td><span class="badge <?php echo $colorcg; ?>"><?php echo number_format($scg/10,'2',',','.'); ?></span></td>
    <td><span class="badge <?php echo $colorcgg; ?>"><?php echo number_format($scgg/10,'2',',','.'); ?></span></td>
  
    <td><span class="badge <?php echo $colorsdep; ?>"><?php echo number_format($sdep/20,'2',',','.'); ?></span></td>
    <td><span class="badge <?php echo $colorsdk; ?>"><?php echo number_format($sdk/30,'2',',','.'); ?></span></td>
    <td><span class="badge <?php echo $colorsds; ?>"><?php echo number_format($sds/504,'2',',','.'); ?></span></td>
  </tr>
<?php } ?>
</table>
