...
class Laporangudanglogistik extends CI_Controller{
  ...
  function __construct(){
    parent::__construct();
    check_login();
    $this->load->model(array('Model_laporanlogistik')); 
  }
  ...
  function cetak_pengeluaran(){
    ...
    $data['dari']             = $dari;
    $data['sampai']           = $sampai;
    $data['data']             = $this->Model_laporanlogistik->list_detailPengeluaran($dari,$sampai,$kode_dept,$kode_kategori,$kode_barang)->result();
    $data['dept']             = $this->Model_laporanlogistik->getDepartemen()->result();
    $data['kategori']         = $this->Model_laporanlogistik->getKategori()->result();
    if(isset($_POST['export'])){
      // Fungsi header dengan mengirimkan raw data excel
